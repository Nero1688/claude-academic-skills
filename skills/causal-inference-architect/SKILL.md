...
3. **事件研究圖是必需品不是裝飾**:前期係數應近 0(pre-trends),圖要含信賴區間;
   ...
   仍成立」。
4. 全部交給 management-figure 的 `event_study_plot()` 出投稿級事件研究圖（參考期、處理時點線、前期陰影、圖註自動印前期聯合檢定 p 與 M̄；估計量名稱進圖註）。
...
上表是最低限度。每種策略(交錯 DiD/IV/RDD/SCM/短窗事件研究)的完整穩健性矩陣——
...
「你的證據」欄直接填 `references/robustness-battery.md` 的表號;攻防表的嚴重度對照
`q1-journal-reviewer` 的 `references/top-journal-standards.md` 四軸(識別/穩健性/經濟量級/
理論貢獻)——審稿人是用那四軸判生死,攻防表就照那四軸備。
