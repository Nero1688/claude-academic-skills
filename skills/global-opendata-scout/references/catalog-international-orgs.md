...
- **base**：`http://data.un.org/WS/rest`
  > 🔒 **這個端點只有 HTTP，沒有 HTTPS**（實測 https 回 500）。明文傳輸代表回應內容
  > 在傳輸途中**可被竄改而不會被察覺**，且中間節點看得到你查了什麼。因此：
  > (1) 絕不在此端點附帶任何金鑰或身分資訊；
  > (2) 取回的數字若要寫進論文，**務必與另一個 HTTPS 來源交叉核對**（World Bank／IMF 多有重疊指標）；
  > (3) 在資料附錄記錄抓取日期與來源協定，供審稿人判斷。
