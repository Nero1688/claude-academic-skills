...
- **對帳表固定加一列「其中下市公司保留 N 家（公司-年 M 筆）／若排除，理由」**：TEJ 主表只含現存公司，下市公司在「各年度下市檔」，未合併就沒有這列可填——此時停下來回頭合併，不得寫 0 或空白。排除下市公司＝存活偏誤，只有研究問題明文限定存續公司才可排除，且理由要進資料節（依據：`tej-data-scout` tej-catalog Part D「存活偏誤與回填」；審稿端 `q1-journal-reviewer` top-journal-standards 與 `causal-inference-architect` robustness-battery 都會問這一列）。
...
6. **列數對帳表**（原始 N → 最終 N，每步差額與原因；固定含「其中下市公司保留 N 家／若排除，理由」列）。
7. 3 筆人工抽核對照。
